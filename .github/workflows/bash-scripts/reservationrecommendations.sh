...
source_file_name="${data_source}_${subscription_name}_$(date -d '1 day ago' +%y%m%d-%H%M%S).json"
...
destination_path="$data_source/$(date -d '1 day ago' +%Y/%m/%d)" # This creates a /YY/MM/DD  folder structure to where the file will be uploaded eg: /23/06/14/[uploaded_file]
