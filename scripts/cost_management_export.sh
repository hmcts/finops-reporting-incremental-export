...
export_name="${data_source}-$(${date_command} +%Y-%m-%d-%H%M)"
start_period=$(${date_command}  -d '1 day ago' +"%Y-%m-%d"T00:00:00+00:00Z)
end_period=$(${date_command}  -d '1 day ago' +"%Y-%m-%d"T23:59:59+00:00Z)
